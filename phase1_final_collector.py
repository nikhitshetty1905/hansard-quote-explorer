# phase1_final_collector.py
# Final Phase 1 collector with all core improvements implemented
...
from fixed_quote_logic import FixedQuoteExtractor

class Phase1FinalCollector:
    """Final Phase 1 collector with word-based filtering and precise proximity testing"""
    
    def __init__(self, db_path="hansard_phase1.db", cache_dir="hansard_cache"):
        ...
        # Use fixed quote extractor
        self.quote_extractor = FixedQuoteExtractor()
        
        # Debate title filtering patterns
    ...
    def fetch_sitting_json(self, year, month, day, use_cache=True):
        """Fetch sitting JSON with caching"""
        try:
    ...
    def process_debate_html(self, debate_info, sitting_url):
        """Process debate HTML using Phase 1 logic"""
        url = debate_info["url"]
        ...
        try:
            ...
            # Phase 1: Extract quotes using improved word-based logic
            quotes = self.quote_extractor.extract_word_based_quotes(
                full_text, 
                min_words=80,    # Calibrated threshold
                max_words=450    # From spec
            )
            
            print(f"      Found {len(quotes)} quotes (80-450 words)")
            ...
            for quote_data in quotes:
                ...
                quality = self.calculate_phase1_quality(quote_data)
    ...
    def calculate_phase1_quality(self, quote_data):
        """Phase 1 quality scoring"""
        score = 0
        ...
        # Word count scoring (calibrated for Hansard)
        ...
        total_terms = mig_count + lab_count
        if total_terms >= 6:
            score += 3
        elif total_terms >= 4:
            score += 2
        ...
        # Proximity bonus (closer terms = higher score)
        if distance <= 5:
            score += 2
        elif distance <= 15:
            score += 1
        ...
        if re.search(r'\b(argue|maintain|contend|assert|claim)\b', quote_text):
            score += 1
        if re.search(r'\b(bill|act|legislation|policy)\b', quote_text):
            score += 1
    ...
    def store_speeches(self, speeches):
        """Store speeches with Phase 1 schema"""
        stored = 0
            ...
            try:
                self.db.execute("""
                    ...
                    (dedup_key, date, house, debate_title, member, party, quote, 
                     word_count, mig_term_count, lab_term_count, token_distance,
                     extraction_method, url, sitting_url, phase1_quality)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    ...
                    speech['lab_term_count'], speech['token_distance'],
                    speech['extraction_method'], speech['url'], 
                    speech['sitting_url'], speech['phase1_quality']
                ))
                stored += 1
    ...
    def collect_date_range(self, start_year, end_year, start_month=1, end_month=12):
        """Collect using Phase 1 improvements"""
        print(f"=== PHASE 1 COLLECTION: {start_year}-{end_year} ===")
        print("+ Word-based filtering (80-450 words)")
        print("+ Precise token-window proximity testing (40 tokens)")
        print("+ Enhanced quality scoring")
        print("+ Improved passage extraction")
        ...
        while current_date <= end_date:
            ...
            if sitting_data:
                ...
                for debate in debates:
                    ...
                    if stored > 0:
                        avg_words = sum(s['word_count'] for s in speeches) / len(speeches)
                        avg_quality = sum(s['phase1_quality'] for s in speeches) / len(speeches)
                        print(f"      {debate['title']}: {stored} speeches (avg {avg_words:.0f} words, quality {avg_quality:.1f})")
                    
                    time.sleep(1)
                
                total_speeches += day_speeches
                if day_speeches > 0:
                    print(f"  Day total: {day_speeches} speeches")
            else:
        ...
        print(f"Sitting success: {successful_sittings}/{total_sittings} ({successful_sittings/total_sittings*100:.1f}%)")
        print(f"Relevant debates: {relevant_debates}/{total_debates} ({relevant_debates/total_debates*100:.1f}%)")
        ...
        if total_speeches > 0:
            stats = self.db.execute("""
                SELECT 
                    ...
                    AVG(mig_term_count), AVG(lab_term_count), 
                    AVG(token_distance), AVG(phase1_quality)
                FROM speeches
            """).fetchone()
            
            print(f"Word count: avg {stats[0]:.0f} (range {stats[1]}-{stats[2]})")
            print(f"Terms: avg {stats[3]:.1f} MIG, {stats[4]:.1f} LAB") 
            print(f"Avg proximity: {stats[5]:.1f} tokens")
            print(f"Avg quality: {stats[6]:.1f}")
...
if __name__ == "__main__":
    collector = Phase1FinalCollector()
    
    print("=== TESTING PHASE 1 FINAL COLLECTOR ===")
